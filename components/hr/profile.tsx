...
function TicketRow({ ticket, onClick }: { ticket: Ticket; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left flex items-start gap-3 px-4 py-3 hover:bg-yellow-50 transition-colors border-b border-yellow-100 last:border-0"
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-xs text-gray-400">#{ticket.id} · {new Date(ticket.createdAt).toLocaleDateString()}</span>
        </div>
      </div>
      <span className="text-xs text-gray-400 shrink-0 mt-1">{ticket.department}</span>
    </button>
  )
}
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white border border-yellow-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className={cn("text-xs px-2 py-0.5 rounded-md border font-medium", ROLE_COLORS[currentUser.role])}>
                  {currentUser.role.replace(/_/g, " ")}
                </span>
                <span className="text-xs text-gray-500 bg-yellow-50 border border-yellow-200 px-2 py-0.5 rounded-md">
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            {[
              { label: "Tickets Created", value: myCreated, color: "text-blue-600" },
              { label: "Assigned to Me", value: assignedToMe.length, color: "text-purple-600" },
              { label: "Resolved by Me", value: myResolved, color: "text-green-600" },
            ].map(({ label, value, color }) => (
              <div key={label} className="bg-yellow-50 rounded-xl p-3 text-center border border-yellow-200">
          ...
          <div className="flex border-b border-yellow-200">
            <button
              ...
              className={cn(
                ...
                tab === "assigned"
                  ...
                  : "border-transparent text-gray-400 hover:text-gray-700 hover:bg-yellow-50"
              )}
            >
              <Inbox className="h-4 w-4" />
              Assigned to Me
              {assignedToMe.length > 0 && (
                <span className="text-xs bg-purple-500 text-white px-1.5 py-0.5 rounded-full">{assignedToMe.length}</span>
              )}
            </button>
            <button
            ...
            >
              ...
              {myAssignedTasks.length > 0 && (
                <span className="text-xs bg-blue-500 text-white px-1.5 py-0.5 rounded-full">{myAssignedTasks.length}</span>
              )}
            </button>
          </div>
          ...
          <div>
            {tab === "assigned" ? (
              assignedToMe.length === 0 ? (
                ...
                <div>
                  <p className="text-xs text-gray-400 px-4 py-2 border-b border-yellow-100 bg-yellow-50/50">
